	...
	public class Dcc
	{
		...
		private void HandleResume(IrcUser user, string[] args)
		{
			try
			{
				...
				foreach (var clients in transferCons.Values)
				{
					foreach (DccTransferClient cli in clients)
					{
		...
		public DccSendClient Send(string target, string filename)
		{
			return Send(target, filename, m_defaultTimeout);
		}
		...
		public DccSendClient Send(string target, string filename, TimeSpan timeout)
		{
			...
			var client = new DccSendClient(this, user, file, timeout);
			m_irc.CommandHandler.DccRequest(target,
			                          "SEND {0} {1} {2} {3}",
			                          file.Name.Replace(" ", "_"),
			                          Util.GetTcpAddress(client.Listener.LocalEndPoint.Address),
		...
		public DccChatClient Chat(string target)
		{
			return Chat(target, m_defaultTimeout);
		}
		...
		/// <param name="target">The nick of the user who should receive a file.</param>
		/// <param name="timeout">Specifies how long the listener should wait for an incoming receiver.</param>
		public DccChatClient Chat(string target, TimeSpan timeout)
		{
			IrcUser user = m_irc.GetUser(target);
			if (user == null)
				user = new IrcUser(m_irc, target);
			var client = new DccChatClient(this, user, timeout);
			m_irc.CommandHandler.DccRequest(target,

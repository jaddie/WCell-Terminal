...
namespace Squishy.Irc.Commands
{
	...
	public class SetIpCommand : Command
	{
		public SetIpCommand()
			: base("SetIP")
		{
			Usage = "SetIp <Ip>";
			Description = "Changes your Util.LocalEndPoint address (which is used to bind local Dcc servers to).";
		}

		public override void Process(CmdTrigger trigger)
		{
			Util.SetLocalAddress(IPAddress.Parse(trigger.Args.NextWord()));
		}
	}
	...
	public class DccSendCommand : Command
	{
		...
		public override void Process(CmdTrigger trigger)
		{
			...
			trigger.Irc.Dcc.Send(target, filename);
		}
	}
	...
	public class DccChatCommand : Command
	{
		...
		{
			string nick = trigger.Args.NextWord();
			DccChatClient client = trigger.Irc.Dcc.GetChatClient(nick);
			if (client == null)
				trigger.Irc.Dcc.Chat(nick);
			else

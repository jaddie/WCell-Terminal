...
namespace Squishy.Irc.Protocol
{
	public class IrcProtocol
	{
		...
		/// <summary>
		/// TODO: Think of a good way to let IrcPacketHandlers have IrcPackets instead of Packets
		/// </summary>
		private void SetupHandlers()
		{
			// If its not recognized:
			defaultPacketHandler = delegate(IrcPacket packet) {
				// PING is special
			...
			AddPacketHandler("NOTICE", delegate(IrcPacket packet) {
				var strStream = new StringStream(packet.Content);
				packet.channel = packet.IrcClient.GetChannel(strStream.NextWord());
			...
			// Nick / Quit
			AddPacketHandler("NICK", delegate(IrcPacket packet) { packet.IrcClient.NickNotify(packet.User, packet.Args); });
			AddPacketHandler("QUIT", delegate(IrcPacket packet) { packet.IrcClient.QuitNotify(packet.User, packet.Args); });
			AddPacketHandler("431", delegate(IrcPacket packet) // :Serv 431 Me :No nickname given
									{ packet.IrcClient.InvalidNickNotify("431", "", packet.Args); });
			AddPacketHandler("433", delegate(IrcPacket packet) // :Serv 433 Me <Nick> :Nickname is already in use.
									{
										var strStream = new StringStream(packet.Content);
										strStream.SkipWord();
			...
			AddPacketHandler("INVITE", delegate(IrcPacket packet) { packet.IrcClient.InviteNotify(packet.User, packet.ArgsOrFirstWord); });
			AddPacketHandler("JOIN", delegate(IrcPacket packet) {
				// sometimes the channel is preceeded by ":" to indicate an argument with unknown content, sometimes its not
				packet.IrcClient.JoinNotify(packet.User, packet.ArgsOrFirstWord);
			});

			// cannot join channel:
			AddPacketHandler("485", delegate(IrcPacket packet) {
			...
			AddPacketHandler("TOPIC", delegate(IrcPacket packet) {
			...
			AddPacketHandler("332", delegate(IrcPacket packet) {
			...
			AddPacketHandler("333", delegate(IrcPacket packet) {
			...
			AddPacketHandler("353", delegate(IrcPacket packet) // :Serv 353 Me = Channel :Namelist
									{
										var strStream = new StringStream(packet.Content);
			...
			AddPacketHandler("MODE", delegate(IrcPacket packet) // :Serv MODE Channel Modes [ModeArgs]
									 {
										 var strStream = new StringStream(packet.Content);
										 packet.channel = packet.IrcClient.GetChannel(strStream.NextWord());
										 strStream.Consume(':', 1);
			...
			AddPacketHandler("PART", delegate(IrcPacket packet) {
				var strStream = new StringStream(packet.Content);
				packet.channel = packet.IrcClient.GetChannel(strStream.NextWord());
				packet.IrcClient.PartNotify(packet.User, packet.channel, packet.Args);
			});
			AddPacketHandler("KICK", delegate(IrcPacket packet) {
				var strStream = new StringStream(packet.Content);
				packet.channel = packet.IrcClient.GetChannel(strStream.NextWord());
			...
			AddPacketHandler("324", delegate(IrcPacket packet) // :Serv 324 Me Channel Modes [ModeArgs]
									{
										var strStream = new StringStream(packet.Content);
			...
			AddPacketHandler("329", delegate(IrcPacket packet) // :Serv 329 [Me] #Channel CreationTime
									{
										var strStream = new StringStream(packet.Content);
			...
			AddPacketHandler("352", delegate(IrcPacket packet) {
				// who info
				//strStream.SkipWord();
				var strStream = new StringStream(packet.Content);
				string channame = strStream.NextWord();
			...
			AddPacketHandler("367", delegate(IrcPacket packet) // :Serv 367 Me Channel banMask bannerName banTime
									{
										var content = packet.Content;
										content.SkipWord();
			...
			AddPacketHandler("368", delegate(IrcPacket packet) {
				packet.channel = packet.IrcClient.GetChannel(packet.Content.NextWord());

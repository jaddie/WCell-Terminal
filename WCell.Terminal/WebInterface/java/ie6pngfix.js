	...
	createVmlStyleSheet: function() {
		...
		*/
		...
		styleSheet.addRule(this.ns + '\\:shape', 'position:absolute;');
		 /*
			large negative top value for avoiding vertical scrollbars for large images,
			suggested by James O'Brien, http://www.thanatopsic.org/hendrik/
		 */
	...
	**/
	fix: function(selector) {
		...
		for (var i=0; i<selectors.length; i++) {
			try {
				this.styleSheet.addRule(selectors[i], 'behavior:expression(PNG.fixEl(this))'); /* seems to execute the function without adding it to the stylesheet - interesting... */
			} catch(r) {}
		}
	},
	...
	fixEl: function(el) {
		...
		else if (el.currentStyle.backgroundImage.toLowerCase().search('.png') == -1 && el.currentStyle.backgroundImage.toLowerCase().search('packets.php') == -1) {
			return;
		}
		var lib = PNG;
		el.vml = {color: {}, image: {}};

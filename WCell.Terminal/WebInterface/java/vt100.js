...
VT100.prototype.initializeUserCSSStyles = function() {
  ...
  if (typeof userCSSList != 'undefined') {
    ...
    for (var i = 0; i <= userCSSList.length; ++i) {
                  ...
                  if (entry.tagName == 'LI') {
                    if (++i >= begin) {
                      ...
                      if (i == current) {
                        ...
                        if (!sheet.disabled) {
                          label.innerHTML= '<img src="images/enabled.gif" />' +
                                           label.innerHTML;
                        }
                      } else {
                        sheet.disabled   = true;
      ...
      group                             +=
        '<li>' + (enabled ? '<img src="images/enabled.gif" />' : '') +
                 label +
        '</li>';
    }
...
VT100.prototype.initializeElements = function(container) {
  ...
  this.cursorHeight            = this.lineheight.clientHeight - 1;
...
VT100.prototype.putString = function(x, y, text, color, style) {
  ...
  if (pixelY >= 0) {
    if ($.browser.webkit) {
      this.cursor.style.top           = pixelY - 67 + 'px';
    } else {
      this.cursor.style.top           = pixelY - 71 + 'px';
    }
  } else {
...
VT100.prototype.beep = function() {
  if (this.visualBell) {
    ...
    try {
      ...
      try {
        this.beeper.src = 'images/beep.wav';
      } catch (e) {
      }
    }
  }

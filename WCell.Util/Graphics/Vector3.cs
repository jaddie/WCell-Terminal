...
namespace WCell.Util.Graphics
{
	...
	public struct Vector3 : IEquatable<Vector3>
	{
		...
		/// </summary>
		public void GetPointXY(float angle, float dist, ref Vector3 point)
		{
			point.X = X - (dist * (float)Math.Sin(angle));
			point.Y = Y + (dist * (float)Math.Cos(angle));
		}

		/// <summary>
		/// Gets the Point that lies in the given angle and has dist from this Point (but in the weird WoW coordinate system).
		/// X -> South, Y -> West
		/// </summary>
		public void GetPointYX(float angle, float dist, ref Vector3 point)
		{
			point.X = X + (dist * (float)Math.Cos(angle));
			point.Y = Y + (dist * (float)Math.Sin(angle));
		}
